...
def get_info_from_share_memory(P=None, A=None, B=None, T=None, kpNumArray=None, kpYieldArray=None, kpLocArray=None,
                               kpOrderArray=None, kpDArray=None, kpTimeArray=None):
    if P != None:
...
def record_error_message(process, folder_name, stdoutdata, stderrdata, pattern, rounds):
    ...
    if (
            process.returncode != 0 and process.returncode != 1 and process.returncode != 6 and process.returncode != -11):  # save result
        if total_errors_interleavings[0] < FILE_SAVED_MAXNUM:
            ...
            err_file_name = folder_name + "/" + "Errors" + "/" + str(
                format(total_errors_interleavings[0], "0>6")) + "_" + str(
                format(rounds, "0>7")) + "_" + CrashType.replace(' ', '-')
...
def record_timeout_message(folder_name, pattern):
    if total_timeouts_interleavings[0] < FILE_SAVED_MAXNUM:
        ...
        print("\033[33m\t[Timeout " + str(subprocessTimeout) + "]: NO." + str(
            total_timeouts_interleavings[0]) + "\033[0m")
...
DryRun_Command = ['sudo', 'Dry_Run=1', 'DBDS_Run=0', 'NO_Trace=1', 'ASAN_OPTIONS=abort_on_error=1:detect_leaks=0',
                  'TSAN_OPTIONS=report_bugs=0']  # the command for running program (adding sudo) # skip data race in dry run 'TSAN_OPTIONS=report_bugs=0'
...
if TaskReproduce == 1:
    try:
        with open(reproduceFile, "rb") as f:
            ...
            memmove(ADDR + sizeof(c_ulonglong) + sizeof(c_uint) * 2, byref(c_uint(maxYieldNum)), sizeof(
...
while returnNormal < 3:
    ...
    if (
            process.returncode == 0 or process.returncode == 1 or process.returncode == 6 or process.returncode == -11):  # and A.value != 1 and A.value != 0:
        if A.value > N or B.value > M or check_and_update_kpNumMaxArray(kpNumArray, zero_in_KP):
            ...
            check_and_update_kpNumMaxArray(kpNumArray,
                                           zero_in_KP)  # This function would not be executed in the if condiction
...
if skipEnter == 0:
    ...
    if input_from_user == "debug" or input_from_user == "DEBUG" or input_from_user == "Debug":
        print("--------------------------------------")
        print_Info_from_shm(N, M, TupleFirst, kpNumArray, kpYieldArray, kpLocArray, kpOrderArray, [],
                            default_coverage_hashID, zero_in_KP, True)
        input("Press Enter to continue...")
else:
    pass

print("\n\033[4;36mEnd Dry Run!\033[0m")
...
for iterPeriod in range(2, period_limit + 1):
    ...
    memmove(ADDR + sizeof(c_ulonglong) + sizeof(c_uint) * 2, byref(c_uint(iterPeriod - 1)), sizeof(
        c_uint))  # tell the program how many periods we used. The program can yield those unrealted thread to the last period
    ...
    while True:
        ...
        if scheduler != None:
            ...
            for inter in scheduler:
                ...
                if DBDS_stage == 1 and iterPeriod - 1 >= 100 and full_deep_run == 0:  # for deep bug, explore interesting prefix is enough
                    break
                ...
                if not prefix == []:
                    if is_redundant_with_prefix(inter,
                        ...
                        rounds = rounds - 1
                        continue
                    print("\033[4;36mtest", str(format(rounds, "0>4")) + " for prefix " + str(prefix) + ":\033[0m ",
                          end="")
                else:
                ...
                if VarModeFlag == 0 or iterPeriod == 2:
                    ...
                    if CurdirectInter != []:
                        ...
                        for i in range(0, len(useful_kp_index_list)):
                            pattern[int(useful_kp_index_list[i] / M)][
                                useful_kp_index_list[i] - int(useful_kp_index_list[i] / M) * M] = patternList[i]
...
#here ends the subprocess
                    ...
                    coverage_hashID = calculate_coverage_hashID(kpLocArray, M * (N + zero_in_KP), kpNumArray, N,
                                                                SHARE_ARRAY_SIZE)
                    ...
                    if coverFlag == 1:
                        ...
                        add_to_explore_dict(waited_to_explore, coverage_hashID, scheduler.get_schedule().copy(),
                                            pattern.copy(), A.value, B.value, kpNumList.copy(),
                                            scheduler.get_prefix().copy())
                        add_to_explore_dict(waited_to_explore_save, coverage_hashID, scheduler.get_schedule().copy(),
                                            pattern.copy(), A.value, B.value, kpNumList.copy(),
                                            scheduler.get_prefix().copy())
                    # save the error message
                    if recordERR:
                    ...
                    if DebugInfo:
                        ...
                        print_Info_from_shm(A.value, B.value, T.value, kpNumArray, kpYieldArray, kpLocArray,
                                            kpOrderArray, prefixPrint, coverage_hashID, zero_in_KP, True)

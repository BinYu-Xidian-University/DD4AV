...
int CountLines(char *filename)//统计代码行数
{
    ifstream ReadFile;
    int n=0;
    string line;
...
bool Instru::runOnModule(Module &M) {
	...
	LLVMContext &C = M.getContext();//获取上下文信息
	...
	if (ConcurrencyInsteresting == 1) {
		...
		for (Module::iterator function = M.begin(), FEnd = M.end(); function != FEnd; function++) { //遍历每一个Function
			...
			if (function->isDeclaration() || function->size() == 0){//Declaration 声明 size bb数量 查看是否这个函数仅仅是一个声明或者没有实际的实现这个函数
				continue;
			}
			//排除以上的可能后，剩下的Function都是我们的目标插桩Function
			...
			//这个部分来声明一个IRBuilder类的对象，然后在下一行将刚才的retArg作为参数与已经在开头声明的、从Function.cpp调用的instr_callFunc一起形成函数调用插入
			IRBuilder<> IRB(&(*FIP));
				...
				for(BasicBlock::iterator instruction = basicblock->begin(), IEnd = basicblock->end(); instruction!=IEnd; instruction++) {	//遍历每一条instruction
					...
					if(Instruction *inst = dyn_cast<Instruction>(instruction)) {
						...
						if(inst->getOpcode() == Instruction::Ret) // Ret指令前插
						{
							...
							Value *CurFuncVa = ConstantExpr::getBitCast(&(*function), Type::getInt8PtrTy(C));
							retArg.push_back(CurFuncVa);
							IRBuild.CreateCall(instr_ReturnFunc, retArg);//这里是利用IRBuilder实现了函数调用的插入
						}
						
						if(inst->getOpcode() == Instruction::Call){// 在FunctionCall前面插
							std::string instr_create = "pthread_create";
						...
						for (int i = 0; i < linesNum; i++){
							...
							//这里读出的SourceLoc是否与静态分析(SensitiveOpPass)文件中的结果是否一致？一致的话打印DebugInfo 并且插桩 否则就不插桩了

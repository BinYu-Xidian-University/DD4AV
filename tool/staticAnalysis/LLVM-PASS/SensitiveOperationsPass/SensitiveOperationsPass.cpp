...
bool SensitiveOperations::runOnModule(Module &M) {
  ...
  ifstream read_file;
  read_file.open(ConFileName, ios::binary);//打开Conconfig
      ...
      for (Function::iterator basicblock = function->begin(), BBEnd = function->end(); basicblock != BBEnd; basicblock++) { //遍历每一个Basic Block
        ...
        for(BasicBlock::iterator instruction = basicblock->begin(), IEnd = basicblock->end(); instruction!=IEnd; instruction++) {	//遍历每一条instruction
          ...
          if(Instruction *inst = dyn_cast<Instruction>(instruction)) {
            ...
            /*KrystalRay catch Function(en/disable) and print the type,arg and SourceLoc*/
            if (auto *callInst = dyn_cast<CallInst>(instruction)) {
              // 对于直接调用，getCalledFunction 返回被调用的函数
              Function *calledFunction = callInst->getCalledFunction();
              if (calledFunction && calledFunction->getName() == "enable_isr")
              {
                outs() << "Here is an enable!" << *instruction << "\nline:" << SourceLoc << "\n"; // 可以打印instruction 在源文件的位置
              }                                                                                   // 相应的操作，比如获取调用指令的行号
              if (calledFunction && calledFunction->getName() == "disable_isr")
              {
                outs() << "Here is a disable!" << *instruction << "\nline:" << SourceLoc << "\n"; // 可以打印instruction 在源文件的位置
              }
            }

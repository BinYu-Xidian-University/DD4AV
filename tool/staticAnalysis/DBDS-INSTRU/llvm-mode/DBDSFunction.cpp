...
typedef struct Sched_Info {
	...
	uint16_t kpLoc[SHARE_MEMORY_ARRAY_SIZE];			// the hash ID(sorloc) of Key Point
	uint16_t kpOrder[SHARE_MEMORY_ARRAY_SIZE];			// recored the order of key points
	uint16_t kpdynamicaddress[SHARE_MEMORY_ARRAY_SIZE];	// recorded the dynamic adress  kladd
	uint16_t kpExeTime[SHARE_MEMORY_ARRAY_SIZE];			//recorded the dynamic exe time kladd
} sched_info;
...
static system_clock::time_point firstTime;
...
void __attribute__((constructor)) traceBegin(void) {
	...
	for (int i = 0; i < SHARE_MEMORY_ARRAY_SIZE; i++){
		si -> kpLoc[i] = 0;
		si -> kpOrder[i] = 0;
		si -> kpdynamicaddress[i] = 0;			//kl add
		si -> kpExeTime[i]	= 0;
	}
...
void instr_LOC_string (void *func, uint16_t intHash, uint16_t addressHash) 
{
#ifdef SAME_THREAD_CONSIDER
	...
	if(timeflag==0)
		{firstTime = system_clock::now();
		timeflag=1; }
	currentTime = system_clock::now();
	auto durationTothisKp = duration_cast<microseconds>(currentTime - firstTime);
	double CurrentRunTime = double(durationTothisKp.count()) * microseconds::period::num / microseconds::period::den; //s为单位
	int currentKpTime = (uint64_t)(CurrentRunTime*1000*1000+10);

	if(loopUnwinded && !isMainTid){
		si -> kpLoc[idx_kpMap[thisTid]] = intHash;		// save the hash ID 对1k取模再乘1000再加sorloc
		si -> kpdynamicaddress[idx_kpMap[thisTid]] = addressHash;						// save the hash ID of address
		si -> kpExeTime[idx_kpMap[thisTid]] = currentKpTime;
		// (uint64_t)(CurrentRunTime*1000*1000+1);
